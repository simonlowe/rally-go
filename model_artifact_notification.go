/*
Rally WebService API

Rally Open API docs

API version: v2.0
Contact: support@rallydev.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ArtifactNotification type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ArtifactNotification{}

// ArtifactNotification ArtifactNotification
type ArtifactNotification struct {
	// Class Name
	ClassName *string `json:"ClassName,omitempty"`
	// Creation Date
	CreationDate *string `json:"CreationDate,omitempty"`
	// Description
	Description *string `json:"Description,omitempty"`
	// ID
	ID *int64 `json:"ID,omitempty"`
	// ID Prefix
	IDPrefix *string `json:"IDPrefix,omitempty"`
	// ID Suffix
	IDSuffix *string `json:"IDSuffix,omitempty"`
	// Name
	Name *string `json:"Name,omitempty"`
	// Object ID
	ObjectID *int64 `json:"ObjectID,omitempty"`
	// ObjectUUID
	ObjectUUID *string     `json:"ObjectUUID,omitempty"`
	Project    *ProjectRef `json:"Project,omitempty"`
	// Subject OID
	SubjectOID   *int64           `json:"SubjectOID,omitempty"`
	Subscription *SubscriptionRef `json:"Subscription,omitempty"`
	User         *UserRef         `json:"User,omitempty"`
	// VersionId
	VersionId *string       `json:"VersionId,omitempty"`
	Workspace *WorkspaceRef `json:"Workspace,omitempty"`
	Warnings  []string      `json:"Warnings,omitempty"`
	Errors    []string      `json:"Errors,omitempty"`
}

// NewArtifactNotification instantiates a new ArtifactNotification object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewArtifactNotification() *ArtifactNotification {
	this := ArtifactNotification{}
	return &this
}

// NewArtifactNotificationWithDefaults instantiates a new ArtifactNotification object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewArtifactNotificationWithDefaults() *ArtifactNotification {
	this := ArtifactNotification{}
	return &this
}

// GetClassName returns the ClassName field value if set, zero value otherwise.
func (o *ArtifactNotification) GetClassName() string {
	if o == nil || IsNil(o.ClassName) {
		var ret string
		return ret
	}
	return *o.ClassName
}

// GetClassNameOk returns a tuple with the ClassName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetClassNameOk() (*string, bool) {
	if o == nil || IsNil(o.ClassName) {
		return nil, false
	}
	return o.ClassName, true
}

// HasClassName returns a boolean if a field has been set.
func (o *ArtifactNotification) HasClassName() bool {
	if o != nil && !IsNil(o.ClassName) {
		return true
	}

	return false
}

// SetClassName gets a reference to the given string and assigns it to the ClassName field.
func (o *ArtifactNotification) SetClassName(v string) {
	o.ClassName = &v
}

// GetCreationDate returns the CreationDate field value if set, zero value otherwise.
func (o *ArtifactNotification) GetCreationDate() string {
	if o == nil || IsNil(o.CreationDate) {
		var ret string
		return ret
	}
	return *o.CreationDate
}

// GetCreationDateOk returns a tuple with the CreationDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetCreationDateOk() (*string, bool) {
	if o == nil || IsNil(o.CreationDate) {
		return nil, false
	}
	return o.CreationDate, true
}

// HasCreationDate returns a boolean if a field has been set.
func (o *ArtifactNotification) HasCreationDate() bool {
	if o != nil && !IsNil(o.CreationDate) {
		return true
	}

	return false
}

// SetCreationDate gets a reference to the given string and assigns it to the CreationDate field.
func (o *ArtifactNotification) SetCreationDate(v string) {
	o.CreationDate = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *ArtifactNotification) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *ArtifactNotification) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *ArtifactNotification) SetDescription(v string) {
	o.Description = &v
}

// GetID returns the ID field value if set, zero value otherwise.
func (o *ArtifactNotification) GetID() int64 {
	if o == nil || IsNil(o.ID) {
		var ret int64
		return ret
	}
	return *o.ID
}

// GetIDOk returns a tuple with the ID field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetIDOk() (*int64, bool) {
	if o == nil || IsNil(o.ID) {
		return nil, false
	}
	return o.ID, true
}

// HasID returns a boolean if a field has been set.
func (o *ArtifactNotification) HasID() bool {
	if o != nil && !IsNil(o.ID) {
		return true
	}

	return false
}

// SetID gets a reference to the given int64 and assigns it to the ID field.
func (o *ArtifactNotification) SetID(v int64) {
	o.ID = &v
}

// GetIDPrefix returns the IDPrefix field value if set, zero value otherwise.
func (o *ArtifactNotification) GetIDPrefix() string {
	if o == nil || IsNil(o.IDPrefix) {
		var ret string
		return ret
	}
	return *o.IDPrefix
}

// GetIDPrefixOk returns a tuple with the IDPrefix field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetIDPrefixOk() (*string, bool) {
	if o == nil || IsNil(o.IDPrefix) {
		return nil, false
	}
	return o.IDPrefix, true
}

// HasIDPrefix returns a boolean if a field has been set.
func (o *ArtifactNotification) HasIDPrefix() bool {
	if o != nil && !IsNil(o.IDPrefix) {
		return true
	}

	return false
}

// SetIDPrefix gets a reference to the given string and assigns it to the IDPrefix field.
func (o *ArtifactNotification) SetIDPrefix(v string) {
	o.IDPrefix = &v
}

// GetIDSuffix returns the IDSuffix field value if set, zero value otherwise.
func (o *ArtifactNotification) GetIDSuffix() string {
	if o == nil || IsNil(o.IDSuffix) {
		var ret string
		return ret
	}
	return *o.IDSuffix
}

// GetIDSuffixOk returns a tuple with the IDSuffix field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetIDSuffixOk() (*string, bool) {
	if o == nil || IsNil(o.IDSuffix) {
		return nil, false
	}
	return o.IDSuffix, true
}

// HasIDSuffix returns a boolean if a field has been set.
func (o *ArtifactNotification) HasIDSuffix() bool {
	if o != nil && !IsNil(o.IDSuffix) {
		return true
	}

	return false
}

// SetIDSuffix gets a reference to the given string and assigns it to the IDSuffix field.
func (o *ArtifactNotification) SetIDSuffix(v string) {
	o.IDSuffix = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *ArtifactNotification) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *ArtifactNotification) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *ArtifactNotification) SetName(v string) {
	o.Name = &v
}

// GetObjectID returns the ObjectID field value if set, zero value otherwise.
func (o *ArtifactNotification) GetObjectID() int64 {
	if o == nil || IsNil(o.ObjectID) {
		var ret int64
		return ret
	}
	return *o.ObjectID
}

// GetObjectIDOk returns a tuple with the ObjectID field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetObjectIDOk() (*int64, bool) {
	if o == nil || IsNil(o.ObjectID) {
		return nil, false
	}
	return o.ObjectID, true
}

// HasObjectID returns a boolean if a field has been set.
func (o *ArtifactNotification) HasObjectID() bool {
	if o != nil && !IsNil(o.ObjectID) {
		return true
	}

	return false
}

// SetObjectID gets a reference to the given int64 and assigns it to the ObjectID field.
func (o *ArtifactNotification) SetObjectID(v int64) {
	o.ObjectID = &v
}

// GetObjectUUID returns the ObjectUUID field value if set, zero value otherwise.
func (o *ArtifactNotification) GetObjectUUID() string {
	if o == nil || IsNil(o.ObjectUUID) {
		var ret string
		return ret
	}
	return *o.ObjectUUID
}

// GetObjectUUIDOk returns a tuple with the ObjectUUID field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetObjectUUIDOk() (*string, bool) {
	if o == nil || IsNil(o.ObjectUUID) {
		return nil, false
	}
	return o.ObjectUUID, true
}

// HasObjectUUID returns a boolean if a field has been set.
func (o *ArtifactNotification) HasObjectUUID() bool {
	if o != nil && !IsNil(o.ObjectUUID) {
		return true
	}

	return false
}

// SetObjectUUID gets a reference to the given string and assigns it to the ObjectUUID field.
func (o *ArtifactNotification) SetObjectUUID(v string) {
	o.ObjectUUID = &v
}

// GetProject returns the Project field value if set, zero value otherwise.
func (o *ArtifactNotification) GetProject() ProjectRef {
	if o == nil || IsNil(o.Project) {
		var ret ProjectRef
		return ret
	}
	return *o.Project
}

// GetProjectOk returns a tuple with the Project field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetProjectOk() (*ProjectRef, bool) {
	if o == nil || IsNil(o.Project) {
		return nil, false
	}
	return o.Project, true
}

// HasProject returns a boolean if a field has been set.
func (o *ArtifactNotification) HasProject() bool {
	if o != nil && !IsNil(o.Project) {
		return true
	}

	return false
}

// SetProject gets a reference to the given ProjectRef and assigns it to the Project field.
func (o *ArtifactNotification) SetProject(v ProjectRef) {
	o.Project = &v
}

// GetSubjectOID returns the SubjectOID field value if set, zero value otherwise.
func (o *ArtifactNotification) GetSubjectOID() int64 {
	if o == nil || IsNil(o.SubjectOID) {
		var ret int64
		return ret
	}
	return *o.SubjectOID
}

// GetSubjectOIDOk returns a tuple with the SubjectOID field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetSubjectOIDOk() (*int64, bool) {
	if o == nil || IsNil(o.SubjectOID) {
		return nil, false
	}
	return o.SubjectOID, true
}

// HasSubjectOID returns a boolean if a field has been set.
func (o *ArtifactNotification) HasSubjectOID() bool {
	if o != nil && !IsNil(o.SubjectOID) {
		return true
	}

	return false
}

// SetSubjectOID gets a reference to the given int64 and assigns it to the SubjectOID field.
func (o *ArtifactNotification) SetSubjectOID(v int64) {
	o.SubjectOID = &v
}

// GetSubscription returns the Subscription field value if set, zero value otherwise.
func (o *ArtifactNotification) GetSubscription() SubscriptionRef {
	if o == nil || IsNil(o.Subscription) {
		var ret SubscriptionRef
		return ret
	}
	return *o.Subscription
}

// GetSubscriptionOk returns a tuple with the Subscription field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetSubscriptionOk() (*SubscriptionRef, bool) {
	if o == nil || IsNil(o.Subscription) {
		return nil, false
	}
	return o.Subscription, true
}

// HasSubscription returns a boolean if a field has been set.
func (o *ArtifactNotification) HasSubscription() bool {
	if o != nil && !IsNil(o.Subscription) {
		return true
	}

	return false
}

// SetSubscription gets a reference to the given SubscriptionRef and assigns it to the Subscription field.
func (o *ArtifactNotification) SetSubscription(v SubscriptionRef) {
	o.Subscription = &v
}

// GetUser returns the User field value if set, zero value otherwise.
func (o *ArtifactNotification) GetUser() UserRef {
	if o == nil || IsNil(o.User) {
		var ret UserRef
		return ret
	}
	return *o.User
}

// GetUserOk returns a tuple with the User field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetUserOk() (*UserRef, bool) {
	if o == nil || IsNil(o.User) {
		return nil, false
	}
	return o.User, true
}

// HasUser returns a boolean if a field has been set.
func (o *ArtifactNotification) HasUser() bool {
	if o != nil && !IsNil(o.User) {
		return true
	}

	return false
}

// SetUser gets a reference to the given UserRef and assigns it to the User field.
func (o *ArtifactNotification) SetUser(v UserRef) {
	o.User = &v
}

// GetVersionId returns the VersionId field value if set, zero value otherwise.
func (o *ArtifactNotification) GetVersionId() string {
	if o == nil || IsNil(o.VersionId) {
		var ret string
		return ret
	}
	return *o.VersionId
}

// GetVersionIdOk returns a tuple with the VersionId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetVersionIdOk() (*string, bool) {
	if o == nil || IsNil(o.VersionId) {
		return nil, false
	}
	return o.VersionId, true
}

// HasVersionId returns a boolean if a field has been set.
func (o *ArtifactNotification) HasVersionId() bool {
	if o != nil && !IsNil(o.VersionId) {
		return true
	}

	return false
}

// SetVersionId gets a reference to the given string and assigns it to the VersionId field.
func (o *ArtifactNotification) SetVersionId(v string) {
	o.VersionId = &v
}

// GetWorkspace returns the Workspace field value if set, zero value otherwise.
func (o *ArtifactNotification) GetWorkspace() WorkspaceRef {
	if o == nil || IsNil(o.Workspace) {
		var ret WorkspaceRef
		return ret
	}
	return *o.Workspace
}

// GetWorkspaceOk returns a tuple with the Workspace field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetWorkspaceOk() (*WorkspaceRef, bool) {
	if o == nil || IsNil(o.Workspace) {
		return nil, false
	}
	return o.Workspace, true
}

// HasWorkspace returns a boolean if a field has been set.
func (o *ArtifactNotification) HasWorkspace() bool {
	if o != nil && !IsNil(o.Workspace) {
		return true
	}

	return false
}

// SetWorkspace gets a reference to the given WorkspaceRef and assigns it to the Workspace field.
func (o *ArtifactNotification) SetWorkspace(v WorkspaceRef) {
	o.Workspace = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ArtifactNotification) GetWarnings() []string {
	if o == nil || IsNil(o.Warnings) {
		var ret []string
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetWarningsOk() ([]string, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ArtifactNotification) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []string and assigns it to the Warnings field.
func (o *ArtifactNotification) SetWarnings(v []string) {
	o.Warnings = v
}

// GetErrors returns the Errors field value if set, zero value otherwise.
func (o *ArtifactNotification) GetErrors() []string {
	if o == nil || IsNil(o.Errors) {
		var ret []string
		return ret
	}
	return o.Errors
}

// GetErrorsOk returns a tuple with the Errors field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ArtifactNotification) GetErrorsOk() ([]string, bool) {
	if o == nil || IsNil(o.Errors) {
		return nil, false
	}
	return o.Errors, true
}

// HasErrors returns a boolean if a field has been set.
func (o *ArtifactNotification) HasErrors() bool {
	if o != nil && !IsNil(o.Errors) {
		return true
	}

	return false
}

// SetErrors gets a reference to the given []string and assigns it to the Errors field.
func (o *ArtifactNotification) SetErrors(v []string) {
	o.Errors = v
}

func (o ArtifactNotification) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ArtifactNotification) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ClassName) {
		toSerialize["ClassName"] = o.ClassName
	}
	if !IsNil(o.CreationDate) {
		toSerialize["CreationDate"] = o.CreationDate
	}
	if !IsNil(o.Description) {
		toSerialize["Description"] = o.Description
	}
	if !IsNil(o.ID) {
		toSerialize["ID"] = o.ID
	}
	if !IsNil(o.IDPrefix) {
		toSerialize["IDPrefix"] = o.IDPrefix
	}
	if !IsNil(o.IDSuffix) {
		toSerialize["IDSuffix"] = o.IDSuffix
	}
	if !IsNil(o.Name) {
		toSerialize["Name"] = o.Name
	}
	if !IsNil(o.ObjectID) {
		toSerialize["ObjectID"] = o.ObjectID
	}
	if !IsNil(o.ObjectUUID) {
		toSerialize["ObjectUUID"] = o.ObjectUUID
	}
	if !IsNil(o.Project) {
		toSerialize["Project"] = o.Project
	}
	if !IsNil(o.SubjectOID) {
		toSerialize["SubjectOID"] = o.SubjectOID
	}
	if !IsNil(o.Subscription) {
		toSerialize["Subscription"] = o.Subscription
	}
	if !IsNil(o.User) {
		toSerialize["User"] = o.User
	}
	if !IsNil(o.VersionId) {
		toSerialize["VersionId"] = o.VersionId
	}
	if !IsNil(o.Workspace) {
		toSerialize["Workspace"] = o.Workspace
	}
	if !IsNil(o.Warnings) {
		toSerialize["Warnings"] = o.Warnings
	}
	if !IsNil(o.Errors) {
		toSerialize["Errors"] = o.Errors
	}
	return toSerialize, nil
}

type NullableArtifactNotification struct {
	value *ArtifactNotification
	isSet bool
}

func (v NullableArtifactNotification) Get() *ArtifactNotification {
	return v.value
}

func (v *NullableArtifactNotification) Set(val *ArtifactNotification) {
	v.value = val
	v.isSet = true
}

func (v NullableArtifactNotification) IsSet() bool {
	return v.isSet
}

func (v *NullableArtifactNotification) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableArtifactNotification(val *ArtifactNotification) *NullableArtifactNotification {
	return &NullableArtifactNotification{value: val, isSet: true}
}

func (v NullableArtifactNotification) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableArtifactNotification) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
