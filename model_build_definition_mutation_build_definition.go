/*
Rally WebService API

Rally Open API docs

API version: v2.0
Contact: support@rallydev.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the BuildDefinitionMutationBuildDefinition type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BuildDefinitionMutationBuildDefinition{}

// BuildDefinitionMutationBuildDefinition BuildDefinition
type BuildDefinitionMutationBuildDefinition struct {
	// Description
	Description *string `json:"Description,omitempty"`
	// Name
	Name     *string     `json:"Name,omitempty"`
	Project  *ProjectRef `json:"Project,omitempty"`
	Projects *Collection `json:"Projects,omitempty"`
	// Uri
	Uri       *string       `json:"Uri,omitempty"`
	Workspace *WorkspaceRef `json:"Workspace,omitempty"`
	Warnings  []string      `json:"Warnings,omitempty"`
	Errors    []string      `json:"Errors,omitempty"`
}

// NewBuildDefinitionMutationBuildDefinition instantiates a new BuildDefinitionMutationBuildDefinition object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBuildDefinitionMutationBuildDefinition() *BuildDefinitionMutationBuildDefinition {
	this := BuildDefinitionMutationBuildDefinition{}
	return &this
}

// NewBuildDefinitionMutationBuildDefinitionWithDefaults instantiates a new BuildDefinitionMutationBuildDefinition object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBuildDefinitionMutationBuildDefinitionWithDefaults() *BuildDefinitionMutationBuildDefinition {
	this := BuildDefinitionMutationBuildDefinition{}
	return &this
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *BuildDefinitionMutationBuildDefinition) SetDescription(v string) {
	o.Description = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *BuildDefinitionMutationBuildDefinition) SetName(v string) {
	o.Name = &v
}

// GetProject returns the Project field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetProject() ProjectRef {
	if o == nil || IsNil(o.Project) {
		var ret ProjectRef
		return ret
	}
	return *o.Project
}

// GetProjectOk returns a tuple with the Project field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetProjectOk() (*ProjectRef, bool) {
	if o == nil || IsNil(o.Project) {
		return nil, false
	}
	return o.Project, true
}

// HasProject returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasProject() bool {
	if o != nil && !IsNil(o.Project) {
		return true
	}

	return false
}

// SetProject gets a reference to the given ProjectRef and assigns it to the Project field.
func (o *BuildDefinitionMutationBuildDefinition) SetProject(v ProjectRef) {
	o.Project = &v
}

// GetProjects returns the Projects field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetProjects() Collection {
	if o == nil || IsNil(o.Projects) {
		var ret Collection
		return ret
	}
	return *o.Projects
}

// GetProjectsOk returns a tuple with the Projects field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetProjectsOk() (*Collection, bool) {
	if o == nil || IsNil(o.Projects) {
		return nil, false
	}
	return o.Projects, true
}

// HasProjects returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasProjects() bool {
	if o != nil && !IsNil(o.Projects) {
		return true
	}

	return false
}

// SetProjects gets a reference to the given Collection and assigns it to the Projects field.
func (o *BuildDefinitionMutationBuildDefinition) SetProjects(v Collection) {
	o.Projects = &v
}

// GetUri returns the Uri field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetUri() string {
	if o == nil || IsNil(o.Uri) {
		var ret string
		return ret
	}
	return *o.Uri
}

// GetUriOk returns a tuple with the Uri field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetUriOk() (*string, bool) {
	if o == nil || IsNil(o.Uri) {
		return nil, false
	}
	return o.Uri, true
}

// HasUri returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasUri() bool {
	if o != nil && !IsNil(o.Uri) {
		return true
	}

	return false
}

// SetUri gets a reference to the given string and assigns it to the Uri field.
func (o *BuildDefinitionMutationBuildDefinition) SetUri(v string) {
	o.Uri = &v
}

// GetWorkspace returns the Workspace field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetWorkspace() WorkspaceRef {
	if o == nil || IsNil(o.Workspace) {
		var ret WorkspaceRef
		return ret
	}
	return *o.Workspace
}

// GetWorkspaceOk returns a tuple with the Workspace field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetWorkspaceOk() (*WorkspaceRef, bool) {
	if o == nil || IsNil(o.Workspace) {
		return nil, false
	}
	return o.Workspace, true
}

// HasWorkspace returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasWorkspace() bool {
	if o != nil && !IsNil(o.Workspace) {
		return true
	}

	return false
}

// SetWorkspace gets a reference to the given WorkspaceRef and assigns it to the Workspace field.
func (o *BuildDefinitionMutationBuildDefinition) SetWorkspace(v WorkspaceRef) {
	o.Workspace = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetWarnings() []string {
	if o == nil || IsNil(o.Warnings) {
		var ret []string
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetWarningsOk() ([]string, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []string and assigns it to the Warnings field.
func (o *BuildDefinitionMutationBuildDefinition) SetWarnings(v []string) {
	o.Warnings = v
}

// GetErrors returns the Errors field value if set, zero value otherwise.
func (o *BuildDefinitionMutationBuildDefinition) GetErrors() []string {
	if o == nil || IsNil(o.Errors) {
		var ret []string
		return ret
	}
	return o.Errors
}

// GetErrorsOk returns a tuple with the Errors field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BuildDefinitionMutationBuildDefinition) GetErrorsOk() ([]string, bool) {
	if o == nil || IsNil(o.Errors) {
		return nil, false
	}
	return o.Errors, true
}

// HasErrors returns a boolean if a field has been set.
func (o *BuildDefinitionMutationBuildDefinition) HasErrors() bool {
	if o != nil && !IsNil(o.Errors) {
		return true
	}

	return false
}

// SetErrors gets a reference to the given []string and assigns it to the Errors field.
func (o *BuildDefinitionMutationBuildDefinition) SetErrors(v []string) {
	o.Errors = v
}

func (o BuildDefinitionMutationBuildDefinition) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BuildDefinitionMutationBuildDefinition) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Description) {
		toSerialize["Description"] = o.Description
	}
	if !IsNil(o.Name) {
		toSerialize["Name"] = o.Name
	}
	if !IsNil(o.Project) {
		toSerialize["Project"] = o.Project
	}
	if !IsNil(o.Projects) {
		toSerialize["Projects"] = o.Projects
	}
	if !IsNil(o.Uri) {
		toSerialize["Uri"] = o.Uri
	}
	if !IsNil(o.Workspace) {
		toSerialize["Workspace"] = o.Workspace
	}
	if !IsNil(o.Warnings) {
		toSerialize["Warnings"] = o.Warnings
	}
	if !IsNil(o.Errors) {
		toSerialize["Errors"] = o.Errors
	}
	return toSerialize, nil
}

type NullableBuildDefinitionMutationBuildDefinition struct {
	value *BuildDefinitionMutationBuildDefinition
	isSet bool
}

func (v NullableBuildDefinitionMutationBuildDefinition) Get() *BuildDefinitionMutationBuildDefinition {
	return v.value
}

func (v *NullableBuildDefinitionMutationBuildDefinition) Set(val *BuildDefinitionMutationBuildDefinition) {
	v.value = val
	v.isSet = true
}

func (v NullableBuildDefinitionMutationBuildDefinition) IsSet() bool {
	return v.isSet
}

func (v *NullableBuildDefinitionMutationBuildDefinition) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBuildDefinitionMutationBuildDefinition(val *BuildDefinitionMutationBuildDefinition) *NullableBuildDefinitionMutationBuildDefinition {
	return &NullableBuildDefinitionMutationBuildDefinition{value: val, isSet: true}
}

func (v NullableBuildDefinitionMutationBuildDefinition) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBuildDefinitionMutationBuildDefinition) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
