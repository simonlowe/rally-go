/*
Rally WebService API

Rally Open API docs

API version: v2.0
Contact: support@rallydev.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the CapacityPlanProjectMutationCapacityPlanProject type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CapacityPlanProjectMutationCapacityPlanProject{}

// CapacityPlanProjectMutationCapacityPlanProject CapacityPlanProject
type CapacityPlanProjectMutationCapacityPlanProject struct {
	Assignments *Collection `json:"Assignments,omitempty"`
	// Planned Capacity Count
	PlannedCapacityCount *int64 `json:"PlannedCapacityCount,omitempty"`
	// Planned Capacity Points
	PlannedCapacityPoints *int64        `json:"PlannedCapacityPoints,omitempty"`
	Workspace             *WorkspaceRef `json:"Workspace,omitempty"`
	Warnings              []string      `json:"Warnings,omitempty"`
	Errors                []string      `json:"Errors,omitempty"`
}

// NewCapacityPlanProjectMutationCapacityPlanProject instantiates a new CapacityPlanProjectMutationCapacityPlanProject object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCapacityPlanProjectMutationCapacityPlanProject() *CapacityPlanProjectMutationCapacityPlanProject {
	this := CapacityPlanProjectMutationCapacityPlanProject{}
	return &this
}

// NewCapacityPlanProjectMutationCapacityPlanProjectWithDefaults instantiates a new CapacityPlanProjectMutationCapacityPlanProject object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCapacityPlanProjectMutationCapacityPlanProjectWithDefaults() *CapacityPlanProjectMutationCapacityPlanProject {
	this := CapacityPlanProjectMutationCapacityPlanProject{}
	return &this
}

// GetAssignments returns the Assignments field value if set, zero value otherwise.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetAssignments() Collection {
	if o == nil || IsNil(o.Assignments) {
		var ret Collection
		return ret
	}
	return *o.Assignments
}

// GetAssignmentsOk returns a tuple with the Assignments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetAssignmentsOk() (*Collection, bool) {
	if o == nil || IsNil(o.Assignments) {
		return nil, false
	}
	return o.Assignments, true
}

// HasAssignments returns a boolean if a field has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) HasAssignments() bool {
	if o != nil && !IsNil(o.Assignments) {
		return true
	}

	return false
}

// SetAssignments gets a reference to the given Collection and assigns it to the Assignments field.
func (o *CapacityPlanProjectMutationCapacityPlanProject) SetAssignments(v Collection) {
	o.Assignments = &v
}

// GetPlannedCapacityCount returns the PlannedCapacityCount field value if set, zero value otherwise.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetPlannedCapacityCount() int64 {
	if o == nil || IsNil(o.PlannedCapacityCount) {
		var ret int64
		return ret
	}
	return *o.PlannedCapacityCount
}

// GetPlannedCapacityCountOk returns a tuple with the PlannedCapacityCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetPlannedCapacityCountOk() (*int64, bool) {
	if o == nil || IsNil(o.PlannedCapacityCount) {
		return nil, false
	}
	return o.PlannedCapacityCount, true
}

// HasPlannedCapacityCount returns a boolean if a field has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) HasPlannedCapacityCount() bool {
	if o != nil && !IsNil(o.PlannedCapacityCount) {
		return true
	}

	return false
}

// SetPlannedCapacityCount gets a reference to the given int64 and assigns it to the PlannedCapacityCount field.
func (o *CapacityPlanProjectMutationCapacityPlanProject) SetPlannedCapacityCount(v int64) {
	o.PlannedCapacityCount = &v
}

// GetPlannedCapacityPoints returns the PlannedCapacityPoints field value if set, zero value otherwise.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetPlannedCapacityPoints() int64 {
	if o == nil || IsNil(o.PlannedCapacityPoints) {
		var ret int64
		return ret
	}
	return *o.PlannedCapacityPoints
}

// GetPlannedCapacityPointsOk returns a tuple with the PlannedCapacityPoints field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetPlannedCapacityPointsOk() (*int64, bool) {
	if o == nil || IsNil(o.PlannedCapacityPoints) {
		return nil, false
	}
	return o.PlannedCapacityPoints, true
}

// HasPlannedCapacityPoints returns a boolean if a field has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) HasPlannedCapacityPoints() bool {
	if o != nil && !IsNil(o.PlannedCapacityPoints) {
		return true
	}

	return false
}

// SetPlannedCapacityPoints gets a reference to the given int64 and assigns it to the PlannedCapacityPoints field.
func (o *CapacityPlanProjectMutationCapacityPlanProject) SetPlannedCapacityPoints(v int64) {
	o.PlannedCapacityPoints = &v
}

// GetWorkspace returns the Workspace field value if set, zero value otherwise.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetWorkspace() WorkspaceRef {
	if o == nil || IsNil(o.Workspace) {
		var ret WorkspaceRef
		return ret
	}
	return *o.Workspace
}

// GetWorkspaceOk returns a tuple with the Workspace field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetWorkspaceOk() (*WorkspaceRef, bool) {
	if o == nil || IsNil(o.Workspace) {
		return nil, false
	}
	return o.Workspace, true
}

// HasWorkspace returns a boolean if a field has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) HasWorkspace() bool {
	if o != nil && !IsNil(o.Workspace) {
		return true
	}

	return false
}

// SetWorkspace gets a reference to the given WorkspaceRef and assigns it to the Workspace field.
func (o *CapacityPlanProjectMutationCapacityPlanProject) SetWorkspace(v WorkspaceRef) {
	o.Workspace = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetWarnings() []string {
	if o == nil || IsNil(o.Warnings) {
		var ret []string
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetWarningsOk() ([]string, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []string and assigns it to the Warnings field.
func (o *CapacityPlanProjectMutationCapacityPlanProject) SetWarnings(v []string) {
	o.Warnings = v
}

// GetErrors returns the Errors field value if set, zero value otherwise.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetErrors() []string {
	if o == nil || IsNil(o.Errors) {
		var ret []string
		return ret
	}
	return o.Errors
}

// GetErrorsOk returns a tuple with the Errors field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) GetErrorsOk() ([]string, bool) {
	if o == nil || IsNil(o.Errors) {
		return nil, false
	}
	return o.Errors, true
}

// HasErrors returns a boolean if a field has been set.
func (o *CapacityPlanProjectMutationCapacityPlanProject) HasErrors() bool {
	if o != nil && !IsNil(o.Errors) {
		return true
	}

	return false
}

// SetErrors gets a reference to the given []string and assigns it to the Errors field.
func (o *CapacityPlanProjectMutationCapacityPlanProject) SetErrors(v []string) {
	o.Errors = v
}

func (o CapacityPlanProjectMutationCapacityPlanProject) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CapacityPlanProjectMutationCapacityPlanProject) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Assignments) {
		toSerialize["Assignments"] = o.Assignments
	}
	if !IsNil(o.PlannedCapacityCount) {
		toSerialize["PlannedCapacityCount"] = o.PlannedCapacityCount
	}
	if !IsNil(o.PlannedCapacityPoints) {
		toSerialize["PlannedCapacityPoints"] = o.PlannedCapacityPoints
	}
	if !IsNil(o.Workspace) {
		toSerialize["Workspace"] = o.Workspace
	}
	if !IsNil(o.Warnings) {
		toSerialize["Warnings"] = o.Warnings
	}
	if !IsNil(o.Errors) {
		toSerialize["Errors"] = o.Errors
	}
	return toSerialize, nil
}

type NullableCapacityPlanProjectMutationCapacityPlanProject struct {
	value *CapacityPlanProjectMutationCapacityPlanProject
	isSet bool
}

func (v NullableCapacityPlanProjectMutationCapacityPlanProject) Get() *CapacityPlanProjectMutationCapacityPlanProject {
	return v.value
}

func (v *NullableCapacityPlanProjectMutationCapacityPlanProject) Set(val *CapacityPlanProjectMutationCapacityPlanProject) {
	v.value = val
	v.isSet = true
}

func (v NullableCapacityPlanProjectMutationCapacityPlanProject) IsSet() bool {
	return v.isSet
}

func (v *NullableCapacityPlanProjectMutationCapacityPlanProject) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCapacityPlanProjectMutationCapacityPlanProject(val *CapacityPlanProjectMutationCapacityPlanProject) *NullableCapacityPlanProjectMutationCapacityPlanProject {
	return &NullableCapacityPlanProjectMutationCapacityPlanProject{value: val, isSet: true}
}

func (v NullableCapacityPlanProjectMutationCapacityPlanProject) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCapacityPlanProjectMutationCapacityPlanProject) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
